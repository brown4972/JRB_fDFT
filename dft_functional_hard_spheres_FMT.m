classdef dft_functional_hard_spheres_FMT < dft_functional_hard_spheres
    ...
    methods
        function F = calc_free_energy(obj, box, mesh, components)
            ...
            F = -box.n0.*log(1-box.n3) ...
                    ...
                    + (box.n2.^3 - 3*box.n2.*dot_nv2nv2)./(24*pi*(1-box.n3).^2);
        ...
        function mu = calc_mu(obj, box, mesh, components)
            ...
            obj.dPhi_dn2 = box.n1./Delta + (box.n2.^2 - dot_nv2nv2)./(8*pi*Delta.^2);
            obj.dPhi_dn3 = box.n0./Delta + (box.n1.*box.n2 - dot_nv1nv2)./(Delta.^2) + (box.n2.^3 - 3*box.n2.*dot_nv2nv2)./(12*pi*Delta.^3);
            obj.dPhi_dnv1x = -box.nv2x./Delta;
            obj.dPhi_dnv1y = -box.nv2y./Delta;
            obj.dPhi_dnv1z = -box.nv2z./Delta;
            obj.dPhi_dnv2x = -box.nv1x./Delta - box.n2.*box.nv2x./(4*pi*Delta.^2);
            obj.dPhi_dnv2y = -box.nv1y./Delta - box.n2.*box.nv2y./(4*pi*Delta.^2);
            obj.dPhi_dnv2z = -box.nv1z./Delta - box.n2.*box.nv2z./(4*pi*Delta.^2);
